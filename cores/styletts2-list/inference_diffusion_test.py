...
class Preprocess:
    ...
    def text_preprocess(self, text, n_merge=12):
        ...
        print("Pre-merge chunk", len(text_norm))
        ...
        print("After-merge chunk", len(text_norm))
        return text_norm
...
class StyleTTS2(torch.nn.Module):
    ...
    def __inference(self, phonem, ref_s, steps=5, embedding_scale=1, speed=1, use_diffusion=True):
        device = self.get_device.device
        ...
        with torch.no_grad():
            ...
            if use_diffusion:
                if self.cache_s_prosody is None:
                    ...
                    s_prosody = self.cache_s_prosody*0.3 + s_prosody*0.7
                    s = s*0.7 + s_prosody*0.3
                    self.cache_s_prosody = s_prosody
            else:
    ...
    def generate(self, phonem, style, steps=5, embedding_scale=1, n_merge=16, use_diffusion=True): 
        list_wav        = []
        ...
        for sentence in text_norm:
            print(sentence)
            wav = self.__inference(sentence, style['style'], 
                                                steps=steps, 
                                                embedding_scale=embedding_scale,
                                                speed=style['speed'],
                                                use_diffusion=use_diffusion)
            wav = wav[3000:-3000] #Remove weird pulse and silent tokens
