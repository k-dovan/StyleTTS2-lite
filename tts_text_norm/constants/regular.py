...
class WebsiteRegular:
    REGREX = [
        r"(?i)\b(?:https?:\/\/|ftp:\/\/|www\.|[^\s:=]+@www\.)?((?:\w+\.)+(?:com|au\.uk|co\.in|net|org|info|coop|int|co\.uk|org\.uk|ac\.uk|uk|vn))(\/[^\s]*)?(?=\b|\/|$)"
    ]
...
class MeasureRegular:
    ...
    REGREX_02 = [
        r"(?i)\b(\d{1,2})(m)(\d{1,2})\b"
    ]
    
class CurrencyRegular:    
    REGREX = [
